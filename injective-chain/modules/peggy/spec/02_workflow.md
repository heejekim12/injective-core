...
The responsibility of Signer is to provide confirmations that an `Operator (Orchestrator)` is partaking in bridge activity. Failure to provide these confirmations can result in Peggy jailing the orchestrator's `Validator` for missed valset or batch confirmations.
...
Whenever `Signer` finds that there is an unconfirmed valset update (token batch) present within the `Peggy Module`, it issues a `MsgValsetConfirm` (`MsgConfirmBatch`) as proof that the operating `Validator` is active in bridge activity.
...
If 2/3 of the validators cannot agree on a single `Attestation`, the oracle is halted. This means no new events will be relayed from Ethereum until some of the validators change their votes. There is no currently implemented claim-based penalty for this, with reasoning outlined in the [validator penalties spec](05_jailing.md).
...
2. **Confirming a Valset on Injective:** Each `Operator` is responsible for confirming Valset updates that are created on Injective. The `Signer` process sends these confirmations via `MsgValsetConfirm` by having the validator's delegated Ethereum key sign over a compressed representation of the Valset data. The `Peggy module` verifies the validity of the signature and persists it to its state.
...
5. **Pruning Valsets on Injective:** Once a `ValsetUpdatedEvent` has been observed on Injective and the signing window has passed, older valsets can be pruned from the `Peggy module` state.
6. **Validator Jailing:** Validators that fail to confirm a valset within `SignedValsetsWindow` can be jailed. Read more in the [validator penalties spec](05_jailing.md)
...
7. **Batch Jailing:** Validators are responsible for confirming batches and can be jailed if they fail to do so before `SignedBatchesWindow` expires. Read more in the [validator penalties spec](05_jailing.md).
